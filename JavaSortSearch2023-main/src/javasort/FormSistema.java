...
public class FormSistema extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon("C:\\Users\\Esau\\Downloads\\spotify.png")); // NOI18N
        ...
        jPanel1Layout.setVerticalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addContainerGap())
            .addComponent(lblProx, javax.swing.GroupLayout.DEFAULT_SIZE, 387, Short.MAX_VALUE)
        );
    ...
    private void carregaArquivo() {
        String csvFile = "listeners.csv";
        String line = "";
        String[] leitura = null;
        try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
            while ((line = br.readLine()) != null) {
                leitura = line.split(";");
                Dados listener = new Dados();
                
                    if (leitura.length >= 1) {
                    listener.setArtistName(leitura[0]);
                }
                if (leitura.length >= 2 && isNumeric(leitura[1])) {
                    listener.setListeners(Integer.parseInt(leitura[1]));
                }
                if (leitura.length >= 3 && isNumeric(leitura[2])) {
                    listener.setDailyTrend(Integer.parseInt(leitura[2]));
                }
                if (leitura.length >= 4 && isNumeric(leitura[3])) {
                    listener.setPeak(Integer.parseInt(leitura[3]));
                }
                if (leitura.length >= 5 && isNumeric(leitura[4])) {
                    listener.setPkListeners(Integer.parseInt(leitura[4]));
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
